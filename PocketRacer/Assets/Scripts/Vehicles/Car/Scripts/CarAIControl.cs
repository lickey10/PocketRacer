...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAIControl : MonoBehaviour
    {
        ...
        public float MovementWaitTime = 5f;
        public float BackupTime = 2f; //amount of time to backup when stuck
        private float stopMovingTime;
        private float startMovingTime;
        private float backupStartTime;
        private float backupSteering = 0;
        ...
        private Vector3 startBackingUpLocation;
        ...
        private void FixedUpdate()
        {
            if (m_Target == null || !Driving)
            {
                ...
                m_CarController.Move(0.1f, 0, -1f, 1f);
            }
            else
            {
                ...
                if (stopMovingTime == 0 && m_Rigidbody.velocity.magnitude < 0.1f && Time.time - startMovingTime > 1.5f && backupStartTime == 0)
                    stopMovingTime = Time.time;
                else if (m_Rigidbody.velocity.magnitude > .9f && backupStartTime == 0)// we are moving forward
                    stopMovingTime = 0;

                if ((stopMovingTime > 0 && Time.time > stopMovingTime + MovementWaitTime) || backupStartTime > 0)//we are stopped for the MovementWaitTime or backing up
                {
                    stopMovingTime = 0;
                    ...
                    if (backupStartTime == 0)
                    {
                        ...
                        startBackingUpLocation = transform.root.position;

                        backupSteering = Random.Range(-10, 10);
                        backupSteering = backupSteering / 10;
                    }

                    if (Time.time < backupStartTime + BackupTime)// we are backing up
                    {
                        m_CarController.Move(backupSteering, 0f, -1f, 0f);
                    }
                    else //we have backed up for the BackupTime - start going forwards
                    {
                        if (Vector3.Distance(startBackingUpLocation, transform.root.position) < 1)//we didn't move
                        {
                            //respawn
                            transform.root.gameObject.GetComponent<CarClass.Car>().DestroyVehicle(true);
                        }
                        else//we backed up - start going forward
                        {
                            startMovingTime = Time.time;
                            backupStartTime = 0;
                        }
